...
class Order {
  static async findAll(limit, offset) {
    ...
    // For each order, get the first product image
    const ordersWithImages = await Promise.all(orders.map(async (order) => {
      // Get the first product image for this order
      const [images] = await pool.query(
        ...
        'WHERE oi.Order_idOrder = ? LIMIT 1',
        [order.idOrder]
      );
      
      // Add the image URL to the order object
      return {
        ...order,
        product_image: images.length > 0 ? images[0].Main_Image_Url : null
      };
    }));
