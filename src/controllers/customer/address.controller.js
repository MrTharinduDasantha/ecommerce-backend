...
class AddressController {
    ...
    try {
      ...
      if (!full_name || !address || !city || !country || !mobile_no) {
        return res
          ...
          .json({
            message:
              "All fields are required (full_name, address, city, country, mobile_no)",
          })
      }
  ...
  async updateAddress(req, res) {
    try {
      ...
      const { address, city, country, mobile_no } = req.body

      if (isNaN(customerId) || isNaN(addressId)) {
        return res
          .status(400)
          .json({ message: "Invalid customer ID or address ID" })
      }
      ...
      if (!address || !city || !country || !mobile_no) {
        return res
          .status(400)
          .json({
            message:
              "All fields are required (address, city, country, mobile_no)",
          })
      }
      ...
      if (addressCheck.length === 0) {
        return res
          ...
          .json({
            message:
              "Address not found or not authorized to update this address",
          })
      }
    ...
    try {
      ...
      if (addressCheck.length === 0) {
        return res
          ...
          .json({
            message:
              "Address not found or not authorized to delete this address",
          })
      }
      ...
      if (orderCheck[0].count > 0) {
        return res
          ...
          .json({
            message:
              "Cannot delete address because it is used by existing orders",
          })
      }
    ...
    try {
      ...
      if (addresses.length === 0) {
        return res
          .status(404)
          .json({
            message: "Address not found or not authorized to view this address",
          })
      }
