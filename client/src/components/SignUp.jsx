    ...
    if (isValid) {
      try {
        const response = await fetch(
          "http://localhost:9000/api/auth/customers/request-email-verify",
          {
            method: "POST",
            headers: {
    ...
    if (otpSent) {
      ...
      try {
        const verifyResponse = await fetch(
          "http://localhost:9000/api/auth/customers/verify",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!verifyResponse.ok) {
          throw new Error(verifyData.message || "Failed to verify code")
        }
        setOtpSent(true)
      } catch (error) {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="form-control">
                ...
                <input
                  type="text"
                  id="otp"
                  className={`w-full py-2 border rounded-lg focus:outline-none bg-white text-center text-[#1D372E] ${
