    ...
    const getDetailsContent = () => {
            ...
            <div key="admin-info" className="px-1 py-5">
                ...
                <div className="bg-[#F4F4F4] rounded-lg shadow-sm overflow-hidden p-4">
                    ...
                    <div className="flex flex-col">
                        <span className="text-sm text-gray-600 p-2">Admin Name: <span className="font-medium p-2">{log.Admin_Name || 'N/A'}</span></span>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-sm text-gray-600 p-2">Action: 
                            <span className={`items-center px-2.5 py-0.5 rounded-full w-45 text-sm font-medium p-2 ${getActionStyle(log.action)}`}>
                                {log.action}
                            </span>
                        </span>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-sm text-gray-600 p-2">Timestamp: <span className="font-medium p-2">{new Date(log.timestamp).toLocaleString()}</span></span>
                    </div>
                </div>
            </div>
        );
        ...
        if (log.new_user_info) {
            try {
                    ...
                    detailsContent.push(
                        <div key="new-admin" className="px-1 py-5">
                            <div className="bg-[#F4F4F4] rounded-lg shadow-sm overflow-hidden p-4">
                                <div className="flex items-center gap-2 mb-6">
                                    <div className="w-1 h-6 bg-[#EAFFF7]"></div>
                                    <h3 className="font-semibold text-[#1D372E]">New Admin Details</h3>
                                </div>
                ...
                else if (log.action.startsWith('Updated') || log.action === 'Toggled category status') {
                    const title = {
                        ...
                        <div key="changes" className="px-1 py-5">
                            <div className="bg-[#F4F4F4] rounded-lg shadow-sm overflow-hidden p-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="bg-white p-4 rounded-lg">
                                        <h4 className="text-sm font-medium text-gray-600 mb-3">Original Data</h4>
                                        {Object.entries(details.originalData || {}).map(([key, value]) => (
                                            <div key={key} className="mb-2">
                                                <span className="text-sm font-medium capitalize">{key}: </span>
                                                <span className="text-sm">{typeof value === 'object' ? JSON.stringify(value) : value}</span>
                                            </div>
                ...
                detailsContent.push(
                    <div key="error" className="px-1 py-5">
                        <div className="bg-[#F4F4F4] rounded-lg shadow-sm overflow-hidden p-4">
                            <div className="flex items-center gap-2 mb-6">
                                <div className="w-1 h-6 bg-[#EAFFF7]"></div>
                                <h3 className="font-semibold text-[#1D372E]">Additional Information</h3>
                            </div>
                            <div className="bg-white p-4 rounded-lg">
                                <p className="text-sm text-gray-600">Failed to parse user info: {log.new_user_info}</p>
                            </div>
                        </div>
                    </div>
                );
            ...
            detailsContent.push(
                <div key="device-info" className="px-1 py-5">
                    <div className="bg-[#F4F4F4] rounded-lg shadow-sm overflow-hidden p-4">
                        <div className="flex items-center gap-2 mb-6">
                            <div className="w-1 h-6 bg-[#EAFFF7]"></div>
                            <h3 className="font-semibold text-[#1D372E]">Device Information</h3>
                        </div>
                        <div className="bg-white p-4 rounded-lg">
