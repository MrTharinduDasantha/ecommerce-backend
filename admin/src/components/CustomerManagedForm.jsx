...
const CustomerManagedForm = () => {
    ...
    socket.on("customerUpdated", (updatedCustomer) => {
      setCustomers((prevCustomers) => {
        const updatedCustomers = prevCustomers.map((c) =>
          c.idCustomer === updatedCustomer.idCustomer ? updatedCustomer : c
        );
        ...
        toast.success("Customer updated in real-time!");
        return updatedCustomers;
      });
    });

    socket.on("customerDeleted", ({ idCustomer }) => {
      setCustomers((prevCustomers) =>
        prevCustomers.filter((c) => c.idCustomer !== idCustomer)
      );
      setFilteredCustomers((prevFiltered) =>
        prevFiltered.filter((c) => c.idCustomer !== idCustomer)
      );
      toast.success("Customer deleted in real-time!");
    });
  ...
  useEffect(() => {
    if (showHistoryModal && selectedCustomer) {
      const fetchHistory = async () => {
        try {
          const data = await api.getCustomerHistory(selectedCustomer.idCustomer);
          setHistory(data);
        } catch (error) {
  ...
  const confirmDelete = async () => {
    try {
      ...
      // No need to manually update state, as Socket.IO will handle it
                      ...
                      <tr key={customer.idCustomer} className="border-b border-[#1D372E]">
                        ...
                        <td className="py-2 px-3 text-xs lg:text-sm">
                          <div className="flex items-center space-x-2">
                            <button
                              onClick={() => navigate(`/dashboard/customer/view-customer/${customer.idCustomer}`)}
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div className="space-y-1">
                        <div className="font-medium text-[#1D372E] text-xs">{customer.Full_Name}</div>
                    ...
                    <div className="text-xs text-gray-600 mb-2">{customer.Mobile_No}</div>
            ...
            {history ? (
              <div>
                ...
                {history.orders.map((order) => (
                  <div key={order.idOrder} className="border border-[#1D372E] p-2 rounded-lg mb-2">
                ...
                {history.deliveryAddresses.map((address, index) => (
                  <div key={index} className="border border-[#1D372E] p-2 rounded-lg mb-2">
