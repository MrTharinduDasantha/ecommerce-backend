...
const OrderDetails = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      setSelectedPaymentStatus(data.order.Payment_Status || 'Not Paid');
      setLoading(false);
    } catch (err) {
  ...
  const handleStatusChange = async () => {
    ...
    Swal.fire({
      ...
      text: `Are you sure you want to change the order status from "${orderDetails.order.Status}" to "${selectedStatus}"?`,
      icon: "warning",
  ...
  const handlePaymentStatusChange = async () => {
    if (!selectedPaymentStatus || selectedPaymentStatus === orderDetails.order.Payment_Status) {
      return;
    }
    
    Swal.fire({
      title: "Update Payment Status",
      text: `Are you sure you want to change the payment status from "${orderDetails.order.Payment_Status || 'Not Paid'}" to "${selectedPaymentStatus}"?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#5CAF90",
      cancelButtonColor: "#6B7280",
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg">
        ...
        <div className="bg-gray-50 p-3 sm:p-4 rounded-lg border border-gray-200">
          ...
          <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm">
            ...
            <p className="flex justify-between text-[#1D372E]">
              ...
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${getPaymentStatusColor(order.Payment_Status || 'Not Paid')}`}>
      ...
      <div className="bg-gray-50 p-3 sm:p-4 rounded-lg border border-gray-200 mb-4 sm:mb-6">
        <h2 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3 text-[#1D372E]">Update Payment Status</h2>
        <div className="space-y-3 sm:space-y-4">
          <div className="flex flex-wrap gap-1 sm:gap-2">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center">
            <button
              ...
              disabled={updatingPaymentStatus || selectedPaymentStatus === (order.Payment_Status || 'Not Paid')}
              className={`px-4 py-1 sm:px-6 sm:py-2 rounded text-xs sm:text-sm font-medium mb-2 sm:mb-0 ${
                updatingPaymentStatus || selectedPaymentStatus === (order.Payment_Status || 'Not Paid')
                  ? 'bg-gray-200 text-gray-500 cursor-not-allowed'
                  : 'bg-[#5CAF90] text-white hover:bg-opacity-90'
              }`}
            >
            ...
            {selectedPaymentStatus !== (order.Payment_Status || 'Not Paid') && !updatingPaymentStatus && (
              <span className="ml-0 sm:ml-3 text-[#1D372E] text-xs sm:text-sm">
                Payment status will be updated from <span className="font-medium">{order.Payment_Status || 'Not Paid'}</span> to <span className="font-medium">{selectedPaymentStatus}</span>
              </span>
            )}
          </div>
        </div>
